...
> [!IMPORTANT]
> **想按最近提交时间排序，需要关闭 `useQuickFetch`。**
> 默认值 `useQuickFetch: true` 使用 `ls-remote` 列出分支，速度快，但只能按字母排序。
> 设为 `false` 后，插件只把每个分支的最新提交拉取到 Controller 上的缓存仓库，并按提交时间排序。
> 首次获取需要几秒，之后只下载有变化的分支。
